...
//this file is for drawing plan on Main Panel.
...
public class DrawGraph implements WindowListener  {
//public class PlanTreeFrame extends JFrame{
...
//	String[]				infoStr;
	public int				displayType;
        private final int SUBOP_OFFSET =QUESTConstants.SO_BASE;

	public DrawGraph() 
	{

	}
	void drawGraph(TreeNode root, int planNum, double zoomLevel)
	{
		numOfPlans =1;
...
//		planNumber = new JLabel("Plan "+(planNum+1));
//		planNumber.setFont(new Font(QUESTConstants.TEXT_FONT,Font.BOLD, 20));
		DBInfo db = DBConstants.databases[2];
		treeNames = db.treeNames;
		treecard = db.treecard;
		layoutGraphs(root, planNum, zoomLevel);
	}
	...
	public void layoutGraphs(TreeNode root, int planNum, double zoomLevel) 
	{
		boolean topPanelRequired = false;
		centerPanel.removeAll();
//		centerPanel.setLayout(new GridLayout(1, numOfPlans));

		myGraphs = new PGraph[numOfPlans];
		myGraphs[0] = new PGraph(treeNames, treecard, root, topPanelRequired, null, false, zoomLevel);
		
		centerPanel.add(BorderLayout.CENTER, myGraphs[0]);
		JGraph graph = myGraphs[0].graph;
...
//	}
	...
	boolean checkIfTreesSame(TreeNode root1) {
		if ( root1.getSimilarity() == QUESTConstants.T_IS_SIMILAR ) {
...
//	}
	...
	void getNonMatchingNodes(Vector entries, TreeNode root) {
		if (root.getSimilarity() != QUESTConstants.T_IS_SIMILAR) {
			entries.add(root);
		}
	...
	void setSimilarity(TreeNode root1, TreeNode root2, Hashtable matching)
{
		...
		for (int i=0; i < keys.length; i++) {
			...
			p1.setSimilarity(QUESTConstants.T_IS_SIMILAR);
			p2.setSimilarity(QUESTConstants.T_IS_SIMILAR);
...
//	}
	...
	void getNonExactJoinMatch(TreeNode[] joins1, TreeNode[] joins2, boolean[] idone, boolean[] jdone, Hashtable matching) {
		for (int i=0; i < joins1.length; i++) {
			...
			joins1[i].setSimilarity(QUESTConstants.T_NO_DIFF_DONE);
		}

		for (int j=0; j < joins2.length; j++) {
			if ( jdone[j] == true )
				continue;
			joins2[j].setSimilarity(QUESTConstants.T_NO_DIFF_DONE);
		}
	}
...
//	}
	...
	void setJoinSimType(TreeNode node1, TreeNode node2, int simType) {
		if ( simType == QUESTConstants.T_NP_LR_SIMILAR ) {
			node1.setSimilarity(QUESTConstants.T_NP_LEFT_EQ);
			node2.setSimilarity(QUESTConstants.T_NP_RIGHT_EQ);
		} else if (simType == QUESTConstants.T_NP_RL_SIMILAR ) {
			node1.setSimilarity(QUESTConstants.T_NP_RIGHT_EQ);
			node2.setSimilarity(QUESTConstants.T_NP_LEFT_EQ);
		} else if (simType == QUESTConstants.T_RL_SIMILAR ) {
			node1.setSimilarity(QUESTConstants.T_RIGHT_EQ);
			node2.setSimilarity(QUESTConstants.T_LEFT_EQ);
		} else if ( simType == QUESTConstants.T_LR_SIMILAR ) {
			node1.setSimilarity(QUESTConstants.T_LEFT_EQ);
			node2.setSimilarity(QUESTConstants.T_RIGHT_EQ);
		} else if (simType == QUESTConstants.T_SO_RL_SIMILAR ) {
			node1.setSimilarity(QUESTConstants.T_SO_RIGHT_SIMILAR);
			node2.setSimilarity(QUESTConstants.T_SO_LEFT_SIMILAR);
		} else if ( simType == QUESTConstants.T_SO_LR_SIMILAR ) {
			node1.setSimilarity(QUESTConstants.T_SO_LEFT_SIMILAR);
			node2.setSimilarity(QUESTConstants.T_SO_RIGHT_SIMILAR);
		} else {
